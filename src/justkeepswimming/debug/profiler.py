...
class Profiler:
    ...
    def __init__(self, enabled: bool, history_length: int):
        ...
        self.records: dict[ProfilerScope,
            ...
            with open(self.options.dump_path, "wb") as file:
                logger.info(
                    f"Saving profiler dump to {
                        self.options.dump_path} (profiler version {
                        self.version})...")
                ...
                if not Profiler.version.is_compatible_with(profiler.version):
                    raise ValueError(
                        f"The profiler dump version {
                            profiler.version} is not compatible with the current version {
                            Profiler.version}.")
                logger.info(
                    f"Loaded profiler dump from {filename} (loaded version {
                        profiler.version}, current version {
                        Profiler.version})")
                return profiler
        except Exception as error:
