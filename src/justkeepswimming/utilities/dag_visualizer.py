...
class DAGVisualizer:
        ...
        try:
            ...
            while self.running:
                ...
                if self.current_graph:
                    self._draw_graph(screen, font, NODE_COLOR, NODE_SELECTED_COLOR,
                                   EDGE_COLOR, TEXT_COLOR, READ_COLOR, WRITE_COLOR,
                                   selected_node)
                else:
                    no_data_text = font.render("Waiting for graph data...", True, TEXT_COLOR)
    ...
    def _draw_graph(self, screen, font, node_color, selected_color, edge_color,
                   text_color, read_color, write_color, selected_node: Optional[str]) -> None:
        ...
        for edge in self.current_graph.edges:
            ...
            if from_node and to_node:
                pygame.draw.line(screen, edge_color, from_node.position, 
                               to_node.position, 2)
                
                # Draw arrow head
                self._draw_arrow_head(screen, from_node.position, to_node.position, edge_color)
        ...
        for node_name, node_data in self.current_graph.nodes.items():
            ...
            if node_name == selected_node:
                self._draw_node_details(screen, font, node_data, text_color, 
    ...
    def _draw_node_details(self, screen, font, node_data: NodeData, text_color,
                          read_color, write_color) -> None:
        ...
        pygame.draw.rect(screen, (40, 40, 50), 
                        (detail_x - 5, detail_y - 5, 400, 190))
        pygame.draw.rect(screen, text_color, 
                        (detail_x - 5, detail_y - 5, 400, 190), 2)
